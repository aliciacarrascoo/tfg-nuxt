...
const { data } = await useAsyncData("results", async () => {
  return await client.from("results").select("sha256, mitre_attack, severity, alert, alert_description, context, recomendation");
});
console.log("backend:", data._rawValue.data);
const {sha256, mitre_attack, severity, alert, alert_description, context, recomendation} = data._rawValue.data[0]
const types = {
  1: {
    label: "Low severity",
